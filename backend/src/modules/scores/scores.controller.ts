...
export const saveScoreController = async (req: Request, res: Response) => {
  try {
    ...
    // Validasi payload array
    const parsedData = saveScoreSchema.parse(req.body);

    // Panggil service baru
    const result = await scoresService.saveScore(user_id, parsedData);
...
export const getScoresByMaterialController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (isNaN(material_id))
      return res
        .status(400)
        .json({ status: "error", message: "ID material tidak valid" });
