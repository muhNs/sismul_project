...
export class AuthController {
  static async register(req: Request, res: Response) {
    try {
      ...
      res
        .status(400)
        .json({ status: "error", message: error.message || "Validasi gagal" });
    }
  }
  ...
  static async login(req: Request, res: Response) {
    try {
      ...
      res
        .status(200)
        .json({ status: "success", message: "Login berhasil", data: user });
    } catch (error: any) {
      if (error instanceof ZodError) {
        return res
          .status(400)
  ...
  static async logout(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ status: "error", message: "Gagal melakukan logout" });
    }
  }
  ...
  static async getMe(req: Request, res: Response) {
    try {
      ...
      if (!userId) {
        return res
          .status(401)
          .json({ status: "error", message: "User belum login" });
      }
