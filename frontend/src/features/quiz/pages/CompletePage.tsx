...
export default function CompletePage() {
  ...
  useEffect(() => {
    ...
    const items: ConfettiItem[] = Array.from({ length: 40 }).map((_, i) => ({
      id: i,
    ...
    <>
      ...
      <main className="flex-grow pt-24 pb-32 px-margin-mobile flex flex-col items-center max-w-[800px] mx-auto w-full relative">
        ...
        <div className="fixed inset-0 pointer-events-none z-10 overflow-hidden">
          {confetti.map((c) => (
            <div
              ...
              style={{ left: c.left, backgroundColor: c.color, animationDelay: c.delay, top: "-20px" }}
            />
          ))}
        </div>
        ...
        <div className="text-center mb-8 space-y-2">
          ...
          <p className="text-on-surface-variant text-sm font-medium">Kamu melakukannya dengan hebat!</p>
        </div>
        ...
        <div className="relative w-full max-w-xs mb-8 flex justify-center">
          ...
          <div className="absolute -bottom-4 right-2 z-20 bg-tertiary-container p-3.5 rounded-xl border-b-4 border-tertiary shadow-lg transform rotate-12 flex items-center justify-center">
            <span className="material-symbols-outlined text-white text-[40px]" style={{ fontVariationSettings: "'FILL' 1" }}>emoji_events</span>
          </div>
        </div>

        {/* Score Card */}
        <Card variant="surface" className="w-full p-6 mb-8 bg-white text-center">
          <span className="font-label text-xs font-bold text-on-surface-variant block mb-2 tracking-wider">HASIL AKHIR</span>
          <div className="flex items-center justify-center gap-2 mb-4">
            <span className="font-display text-2xl font-bold text-on-surface">Total Skor:</span>
            <span className="font-display text-3xl font-extrabold text-primary">{quizScore} Poin</span>
          </div>
          <ProgressBar value={quizScore} color="primary" />
          <p className="mt-4 font-label text-sm font-bold text-primary">
            {quizScore >= 60 ? "Hampir Sempurna! Tingkatkan lagi ya!" : "Jangan menyerah, coba lagi!"}
          </p>
        </Card>

        {/* Actions */}
        <div className="w-full space-y-4">
          <Button onClick={() => router.push("/quiz/prep")} variant="primary" className="py-4">
            <span className="material-symbols-outlined">play_arrow</span>
            MAIN LAGI
          </Button>
          <div className="grid grid-cols-2 gap-4">
            <Button onClick={() => router.push("/choose-chapter")} variant="outline" className="py-4">
              <span className="material-symbols-outlined">grid_view</span>
              CHAPTER
            </Button>
            <Button onClick={() => router.push("/")} variant="secondary" className="py-4">
